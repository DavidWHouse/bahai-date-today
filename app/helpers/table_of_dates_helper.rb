...
module TableOfDatesHelper
  def year_row(year)
    ...
    rows << content_tag(:td, gregorian_equivalent_naw_ruz(year))
    rows << content_tag(:td, twin_holy_days(year))
    rows << content_tag(:td, gregorian_equivalent_twin_holy_days(year))
    rows << content_tag(:td, ayyam_i_ha_days(year))
    rows << content_tag(:td, gregorian_equivalent_ayyam_i_ha(year))
    rows.join.html_safe
  end
  ...
  def twin_holy_days(year)
    ...
    if (month1 != month2)
      "#{date1.day.number} #{date1.month.html}, #{date2.day.number} #{date2.month.html}".html_safe
    else
      "#{date1.day.number}, #{date2.day.number} #{date2.month.html}".html_safe
    end
  end
